...
| `graft_raw_224_mcu.onnx` | **the graft's GAP8-compilable graph**: `--mcu-act --raw-head` (raw head dict, decode dropped) | 12 | CP 10.1, 2026-07-15 |
| `yolo11n_pose_224_raw.onnx` | **the baseline's GAP8-compilable graph**: gate donor, `--raw-head` — the matched twin of the above | 12 | CP 10.1, 2026-07-15 |
...
python -m detect.export_grafted_onnx --imgsz 224 --opset 12 --mcu-act --raw-head --out models/res224/graft_raw_224_mcu.onnx
```

The baseline's matched twin (`.venv-nas`; module form — it imports the shared exporter):

```bash
python -m mcu.export_baseline --imgsz 224 --out models/res224/yolo11n_pose_224_raw.onnx
```

Both raw-head graphs expose the same six outputs — `boxes (1,64,1029)`, `scores (1,1,1029)`,
`feats_p3/p4/p5`, `kpts (1,24,1029)` — and the same head config (nc=1, 8 kpts), so a
comparison between them isolates the backbone.
