...
# AutoTiler's generated <prefix>Kernels.h does `#include "<prefix>.h"` and expects the
# app to supply that shim (it only declares the _L3_Flash symbol; the SDK examples ship
# a hand-written one, e.g. mnist.h / classification.h). Generate it -- the Makefile's
# `-I.` puts the app dir on the include path. Same shim cyc_probe.sh writes for the sim.
cat > "${APPDIR}/${MODEL}.h" <<EOF
#ifndef __${MODEL}_APP_H__
#define __${MODEL}_APP_H__
#include "Gap.h"
extern AT_HYPERFLASH_FS_EXT_ADDR_TYPE ${MODEL}_L3_Flash;
#endif
EOF
